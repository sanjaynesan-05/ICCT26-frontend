...
const PlayerDetail = () => {
  ...
  // Get player and team data from navigation state
    ...
    return (
      <div className="min-h-screen bg-gradient-primary flex items-center justify-center">
        <div className="bg-red-500/20 border border-red-500/50 rounded-lg p-6 text-red-200 font-body">
          Player not found. Please navigate from the team page.
        </div>
      </div>
    )
  }
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="font-heading text-3xl text-white tracking-wide">{player.name}</h1>
                <p className="font-body text-accent text-sm">
                  {team ? `${team.teamName} • ${player.playerId}` : player.playerId}
                </p>
              </div>
            </div>
            <button
      ...
      <div className="container mx-auto px-4 py-8 max-w-5xl">
        ...
        >
          <div className="flex items-start gap-8">
            ...
            <div className="bg-gradient-to-br from-accent/30 to-accent/10 rounded-2xl p-8 border-2 border-accent/50">
              <div className="text-center">
                <p className="text-accent text-sm font-body mb-2">Jersey No.</p>
                <p className="font-heading text-7xl text-white tracking-wide">{player.jerseyNumber}</p>
              </div>
            </div>

            {/* Player Details */}
            <div className="flex-1">
              <h2 className="font-heading text-5xl text-white mb-6 tracking-wide">{player.name}</h2>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <p className="text-accent text-sm font-body mb-1">Role</p>
                  <p className="text-white font-body text-lg">{player.role}</p>
                </div>
                <div>
                  <p className="text-accent text-sm font-body mb-1">Age</p>
                  <p className="text-white font-body text-lg">{player.age} years</p>
                </div>
                <div>
                  <p className="text-accent text-sm font-body mb-1">Phone</p>
                  <p className="text-white font-body text-lg">{player.phone}</p>
                </div>
                {player.email && (
                  <div>
                    <p className="text-accent text-sm font-body mb-1">Email</p>
                    <p className="text-white font-body text-lg break-all">{player.email}</p>
                  </div>
                )}
                <div>
                  <p className="text-accent text-sm font-body mb-1">Player ID</p>
                  <p className="text-white font-body text-lg">{player.playerId}</p>
                </div>
                {team && (
                  <div>
                    <p className="text-accent text-sm font-body mb-1">Team</p>
                    <p className="text-white font-body text-lg">{team.teamName}</p>
                    <p className="text-white/60 text-sm font-body">{team.churchName}</p>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="bg-white/5 rounded-lg p-6 border border-white/10">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-white font-body text-lg">Aadhar Card</p>
                      <p className="text-white/60 text-sm font-body">Identity Proof Document</p>
                    </div>
                  </div>
                  <a
                    href={player.aadharFile}
                    target="_blank"
              ...
              <div className="bg-white/5 rounded-lg p-6 border border-white/10">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-white font-body text-lg">Subscription Card</p>
                      <p className="text-white/60 text-sm font-body">Church Membership Proof</p>
                    </div>
                  </div>
                  <a
                    href={player.subscriptionFile}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="btn-gold px-6 py-3 rounded-lg font-body inline-flex items-center gap-2"
                  >
          ...
          <div className="mt-6 pt-6 border-t border-white/20">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {player.aadharFile && player.subscriptionFile ? (
                  <>
